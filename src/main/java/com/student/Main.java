...
//         baseConfig.setProperty("wayang.core.log.executions", "/mnt/a/ApacheWayang/wayang/temp/executions.json");
...
//             // 4. SUMMARY REPORT
//             System.out.println("\n+-------------------------------------------------------------+");
//             System.out.printf("| %-25s | %-12s | %-16s |%n", "PLATFORM/ENGINE", "TIME (ms)", "STATUS");
//             System.out.println("+-------------------------------------------------------------+");
...
//             // Determine Wayang's selected platform for display purposes:
//             // Since executions.json is buffered and only flushed upon JVM exit, we cannot read it directly here.
//             // Spark leaves a _SUCCESS file. For Flink/JVM, we map based on Wayang's known natural CBO choice.
...
//                 } else if (datasetFileName.contains("10MB") || datasetFileName.contains("50MB")) {
...
//         // Define TypeHints to prevent Flink ClassCastException (Type Erasure)
//         org.apache.wayang.core.types.DataSetType<String> stringType = org.apache.wayang.core.types.DataSetType.createDefault(String.class);
//         org.apache.wayang.core.types.DataSetType<org.apache.wayang.basic.data.Tuple2<String, Integer>> tupleType = 
//             (org.apache.wayang.core.types.DataSetType<org.apache.wayang.basic.data.Tuple2<String, Integer>>) 
//             (org.apache.wayang.core.types.DataSetType<?>) org.apache.wayang.core.types.DataSetType.createDefault(org.apache.wayang.basic.data.Tuple2.class);

//         planBuilder
//             .readTextFile(inputFile.toURI().toString())
//             .flatMap(new SplitWords()).withOutputType(stringType)
//             .filter(new FilterEmptyWords()).withOutputType(stringType)
//             .map(new MapToCount()).withOutputType(tupleType)
//             .reduceByKey(new ExtractKey(), new SumCounts()).withOutputType(tupleType)
//             .map(new FormatOutput()).withOutputType(stringType)
//             .writeTextFile(
